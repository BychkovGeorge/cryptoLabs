...
public class AesBlockEncrypt {
    ...
    public static byte[] aesBlockDecrypt(SecretKey key, byte[] data, boolean isFinalBLock, String paddingType) throws NoSuchPaddingException, NoSuchAlgorithmException, BadPaddingException, IllegalBlockSizeException, InvalidKeyException, NoSuchProviderException {
        if (isFinalBLock) {
            Cipher aesCipher = Cipher.getInstance(paddingType);
